const PRODUCTOS = [...(ventana.BASIC_PRODUCTS||[]),
  ...
  {id:'p-triple',name:'Prueba triple Influenza A y B, SARS-CoV-2 (COVID) y Virus Sincitial',detail:'Precio por pieza',price:100,category:'Otros productos',icon:'🧬'},
  {id:'p-cuadruple',name:'Prueba cuádruple Influenza A y B, SARS-CoV-2 (COVID), Virus Sincitial y Adenovirus',detail:'Precio por pieza',price:120,category:'Otros productos',icon:'🧬'},
  ...
  {id:'p-guantes-nitrilo',name:'Caja de guantes de nitrilo',detail:'Caja; seleccione talla en Guantes por talla',price:160,category:'Otros productos',icon:'🧤'},
  {id:'p-guantes-ambiderm',name:'Caja de guantes Ambiderm estéril',detail:'Caja; seleccione talla en Guantes por talla',price:160,category:'Otros productos',icon:'🧤'},
...
/* Corrección final: Guantes por talla, retiro de suplementos y carrito */
(function(){
  ...
  function resolveCategory(element){
    const raw=element?.dataset?.category || element?.getAttribute?.('data-category') || element?.textContent || '';
    const value=normalizeText(raw);

    if(value.includes('guante')) return 'Guantes';
    if(value.includes('medicamento especial')) return 'Medicamento especial';
    if(value.includes('medicamento basico')) return 'Medicamento básico';
    if(value.includes('otro') || value.includes('producto')) return 'Otros productos';
    if(value.includes('diabetes')) return 'Diabetes';
    if(value.includes('vitamina') || value.includes('suplemento')) return '__REMOVE__';
    return raw.trim();
  }

  function showCategory(category){
    if(!category || category==='__REMOVE__') return;
    state.category=category;
    state.page=1;

    const select=document.getElementById('categoryFilter');
    if(select){
      const option=[...select.options].find(o=>o.value===category);
      if(option) select.value=category;
    }
  ...
  function findCartDrawer(){
    return document.getElementById('cartDrawer')
      || document.getElementById('carritoDrawer')
  ...
  function bindFinalControls(){
    ...
    categoryCandidates.forEach(card=>{
      ...
      if(card.dataset.finalCategoryBound==='1') return;
      card.dataset.finalCategoryBound='1';
    ...
    const cartButtons=[
      ...document.querySelectorAll(
        '#cartButton,#cartBtn,#carritoButton,#btnCarrito,[data-open-cart],.cart-button,.cart-trigger'
      )
    ];

    // Fallback: any button/link whose visible text says Carrito.
    document.querySelectorAll('button,a').forEach(el=>{
      if(normalizeText(el.textContent).includes('carrito') && !cartButtons.includes(el)){
        cartButtons.push(el);
      }
    });

    cartButtons.forEach(button=>{
      if(button.dataset.finalCartBound==='1') return;
      button.dataset.finalCartBound='1';
      button.addEventListener('click',event=>{
        event.preventDefault();
        event.stopImmediatePropagation();
        openCart();
    ...
    // Make every rendered Add button reliable.
    document.querySelectorAll('.add-product,[data-product-id]').forEach(button=>{
      if(!button.dataset.productId || button.dataset.finalAddBound==='1') return;
      button.dataset.finalAddBound='1';
      button.addEventListener('click',event=>{
        event.preventDefault();
        event.stopImmediatePropagation();
        const qtyInput=document.querySelector(`[data-qty-for="${button.dataset.productId}"]`);
  ...
  if(typeof renderProducts==='function'){
    const wrappedRenderProducts=renderProducts;
    window.renderProducts=function(){
      const result=wrappedRenderProducts.apply(this,arguments);
      setTimeout(bindFinalControls,0);
      return result;
    };
    // Update the global lexical reference where browsers permit it.
    try{ renderProducts=window.renderProducts; }catch(_){}
  }
  ...
  setInterval(bindFinalControls,1000);
})();
