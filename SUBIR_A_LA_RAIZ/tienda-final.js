...
function setupCheckout(){
  ...
  $('#checkoutForm').addEventListener('submit',async e=>{
    ...
    try{
      ...
      const order={folio,fecha:new Date().toISOString(),status:'pending',cliente:data,productos:items.map(x=>({id:x.product.id,nombre:x.product.name,cantidad:x.qty,precioEstimado:x.product.price})),subtotalEstimado:subtotal,estado:'Pedido recibido',avisoPrecio:'El importe final se confirma cuando el pedido es preparado en el almacén. Entrega a domicilio sin costo en pedidos desde $1,000.00 en la Zona Metropolitana y desde $1,200.00 en envíos foráneos. En pedidos de menor importe se aplicará un cargo de entrega de $250.00.'};
...
/* Actualización autorizada del aviso de entrega */
(() => {
  const title = "Precio estimado";
  const messageHtml = `
  ...
  function updateNotice() {
    const elements = document.querySelectorAll("div, section, aside, p");
    for (const element of elements) {
      const text = normalize(element.textContent);
      const isOldNotice =
        text.includes("precio estimado") &&
        (
          text.includes("el importe final se confirma cuando el producto llega desde el almacen") ||
          text.includes("el precio final puede variar y se confirma al recibir el producto desde almacen")
        );

      if (isOldNotice) {
        element.innerHTML = messageHtml;
        element.setAttribute("data-delivery-notice-updated", "true");
        return true;
      }
    }
    return false;
